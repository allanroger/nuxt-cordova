...
const ROOT_CONTAINER = '{00000000-0000-0000-FFFF-FFFFFFFFFFFF}';
const DEVICE_CLASS_KEY = '{A45C254E-DF1C-4EFD-8020-67D146A850E0},10';
const DEVICE_CLASS_KEY_NO_SEMICOLON = '{A45C254E-DF1C-4EFD-8020-67D146A850E0}10';
const ROOT_CONTAINER_QUERY = 'System.Devices.ContainerId:="' + ROOT_CONTAINER + '"';
const HAL_DEVICE_CLASS = '4d36e966-e325-11ce-bfc1-08002be10318';
const DEVICE_DRIVER_VERSION_KEY = '{A8B865DD-2E3D-4094-AD97-E593A70C75D6},3';
...
module.exports = {
  ...
  getDeviceInfo: function (win, fail, args) {
    ...
    if (localSettings.values.deviceId) {
      ...
      try {
        const ASHWID = Windows.System.Profile.HardwareIdentification.getPackageSpecificToken(null).id;
    ...
    const devicePlatform = userAgent.indexOf('MSAppHost/1.0') == -1 ? 'windows' : 'windows8';
    let versionString = userAgent.match(/Windows (?:Phone |NT )?([0-9.]+)/)[1];

    const deviceInfo = new Windows.Security.ExchangeActiveSyncProvisioning.EasClientDeviceInformation();
    ...
    const isVirtual = Windows.System.RemoteDesktop.InteractiveSession.isRemote || deviceInfo.systemProductName == 'Virtual';
    const manufacturer = deviceInfo.systemManufacturer;
    const model = deviceInfo.systemProductName;

    const Pnp = Windows.Devices.Enumeration.Pnp;
    ...
    Pnp.PnpObject.findAllAsync(Pnp.PnpObjectType.device,
        ...
        .then(function (rootDevices) {
          for (var i = 0; i < rootDevices.length; i++) {
            const rootDevice = rootDevices[i];
